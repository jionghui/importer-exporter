...
public class Underground extends KmlGenericObject{

	public static final String STYLE_BASIS_NAME = ""; // "Underground"

	public Underground(Connection connection,
	...
	protected List<DisplayForm> getDisplayForms() {
		return config.getProject().getKmlExporter().getBuildingDisplayForms();		// Temp using building's settings 
	}																				// TODO: ADD UNDERGROUND SETTINGS
	...
	protected String getHighlightingQuery() {
		return Queries.getUndergroundPartHighlightingQuery(currentLod);
	}
	...
	public void read(KmlSplittingResult work) {
		...
		ResultSet rs = null;
		try {
			psQuery = connection.prepareStatement(Queries.UNDERGROUND_PARTS_FROM_UNDERGROUND);
			...
			while (rs.next()) {
				long undergroundPartId = rs.getLong(1);
				List<PlacemarkType> placemarkBPart = readUndergroundPart(undergroundPartId, work);
			...
			Logger.getInstance().error("SQL error while getting underground parts for underground " + work.getGmlId() + ": " + sqlEx.getMessage());
			return;
		}
		finally {
	...
	private List<PlacemarkType> readUndergroundPart(long undergroundPartId, KmlSplittingResult work) {
		...
		try {
			...
			while (currentLod >= minLod) {
				...
				try {					
					psQuery = connection.prepareStatement(Queries.getUndergroundPartQuery(currentLod, work.getDisplayForm(), lod0FootprintMode, databaseAdapter.getDatabaseType()),
							ResultSet.TYPE_SCROLL_INSENSITIVE,
							ResultSet.CONCUR_READ_ONLY);
					for (int i = 1; i <= psQuery.getParameterMetaData().getParameterCount(); i++) {
						psQuery.setLong(i, undergroundPartId);
					}
					rs = psQuery.executeQuery();
					if (rs.isBeforeFirst()) {
				...
				if ((rs == null && currentLod > 1 && (work.getDisplayForm().getForm() <= DisplayForm.EXTRUDED)) || 
					...
					int groupBasis = 4;
					try {
						psQuery = connection.prepareStatement(Queries.getUndergroundPartAggregateGeometries(0.001,
			...
			if (rs != null) { // result not empty
				...
				case DisplayForm.EXTRUDED:
					...
					for (int i = 1; i <= psQuery2.getParameterMetaData().getParameterCount(); i++) {
						psQuery2.setLong(i, undergroundPartId);
					}
				...
				case DisplayForm.GEOMETRY:
					...
					if (work.getDisplayForm().isHighlightingEnabled()) {
						if (config.getProject().getKmlExporter().getFilter().isSetComplexFilter()) { // region
							List<PlacemarkType> hlPlacemarks = createPlacemarksForHighlighting(undergroundPartId, work);
							hlPlacemarks.addAll(createPlacemarksForGeometry(rs, work));
							return hlPlacemarks;
						}
						else { // reverse order for single undergrounds
							List<PlacemarkType> placemarks = createPlacemarksForGeometry(rs, work);
							placemarks.addAll(createPlacemarksForHighlighting(undergroundPartId, work));
							return placemarks;
						}
					}
				...
				case DisplayForm.COLLADA:
					fillGenericObjectForCollada(rs, config.getProject().getKmlExporter().getBuildingColladaOptions().isGenerateTextureAtlases()); // fill and refill TODO: ADD CONFIG FOR UNDERGROUND
					...
					try {
						if (work.getDisplayForm().isHighlightingEnabled()) {
							return createPlacemarksForHighlighting(undergroundPartId, work);
						}
						// just COLLADA, no KML
	...
	protected List<PlacemarkType> createPlacemarksForHighlighting(long undergroundPartId, KmlSplittingResult work) throws SQLException {
		...
		try {
			...
			for (int i = 1; i <= getGeometriesStmt.getParameterMetaData().getParameterCount(); i++) {
				// this is THE LINE
				getGeometriesStmt.setLong(i, undergroundPartId);
			}
