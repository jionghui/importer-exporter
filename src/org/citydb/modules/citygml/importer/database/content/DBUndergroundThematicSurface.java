...
public class DBUndergroundThematicSurface implements DBImporter {
	...
	public DBUndergroundThematicSurface(Connection batchConn, DBImporterManager dbImporterManager) throws SQLException {
	...
	private void init() throws SQLException {
		...
		.append("insert into UNDERGROUND_THEMATIC_SURFACE (ID, OBJECTCLASS_ID, UNDERGROUND_ID, LOD2_MULTI_SURFACE_ID, LOD3_MULTI_SURFACE_ID, LOD4_MULTI_SURFACE_ID) values ")
		.append("(?, ?, ?, ?, ?, ?)");
	...
	public long insert(AbstractBoundarySurface boundarySurface, CityGMLClass parent, long parentId) throws SQLException {
		...
		for (int i = 0; i < 3; i++) {
				...
				if (multiSurfaceProperty.isSetMultiSurface()) {
					...
					if (href != null && href.length() != 0) {
						dbImporterManager.propagateXlink(new DBXlinkSurfaceGeometry(
    							href, 
    							boundarySurfaceId, 
    							TableEnum.UNDERGROUND_THEMATIC_SURFACE, 
			...
			if (multiSurfaceId != 0)
				psThematicSurface.setLong(4 + i, multiSurfaceId);
			else
				psThematicSurface.setNull(4 + i, Types.NULL);
		}

		psThematicSurface.addBatch();
		if (++batchCounter == dbImporterManager.getDatabaseAdapter().getMaxBatchSize())
			dbImporterManager.executeBatch(DBImporterEnum.UNDERGROUND_THEMATIC_SURFACE);
	...
	public DBImporterEnum getDBImporterType() {
		return DBImporterEnum.UNDERGROUND_THEMATIC_SURFACE;
	}
